/* =======================================================
Fiji Macro: Batch Merge Series Channels & 3D to 2D
- Liner adjjustment via enchancing contrast to Enhance Contrast 35% ??
- uniformly enhancing for all images to ?? increase Contrast 35% 
======================================================= */
// 3. Get information about the file
run("Bio-Formats Macro Extensions");

//
channelColors = newArray("", "Blue", "Yellow Hot", "Magenta Hot", "Cyan");
...
//soloChannelMerged(1,true,50,0); // #nuclei
//soloChannelMerged(2,false,20,150); //myosin
//soloChannelMerged(3,true,15,120); // glut4
//soloChannelMerged(4,true,20,150); // titin

//glut4 only
//overlayChannelWithNuclei(3,true,15,120);
...
function soloChannelMerged(channel, despeckle, min, max){
	// 2. Set Batch Mode for speed
	// 20x faster; bypassess overhead of rendering images to the display.
	setBatchMode(true); 
	...
	for (s = 1; s <= seriesCount; s++) {
		 showProgress(s, seriesCount);
				...
				run("Merge Channels...", "c" + channel + "=[ch" + channel + "_2d] create");
				 ...
				 // 8. Save and Close
				 saveAs("Tiff", outputDir + File.nameWithoutExtension + "_series" + (s) + "_ch"+ channel +"_merged.tif");
	}
...
function overlayChannel2andChannel4WithNuclei(){
	// 2. Set Batch Mode for speed
	// 20x faster; bypassess overhead of rendering images to the display.
	setBatchMode(true); 
	...
	for (s = 1; s <= seriesCount; s++) {
		 showProgress(s, seriesCount);
				...
				run("Merge Channels...", "c1=[ch1_2d] c2=[ch2_2d] c4=[ch4_2d] create");
				 ...
				 // 8. Save and Close
				 saveAs("Tiff", outputDir + File.nameWithoutExtension + "_series" + (s) + "_ch2_ch4_with_nuclei_overlay.tif");
	}
	close("*"); // Closes all images
	setBatchMode(false);
	setBatchMode("exit & display"); //to force open all images that were processed.
...
function overlayChannelWithNuclei(channel, despeckle, min, max){
	...
	for (s = 1; s <= seriesCount; s++) {
		 showProgress(s, seriesCount);
			    ...
			    // Function to clean a channel: (ImageName, MinThreshold, MaxBrightness)
				cleanChannel("ch1_2d", 50, 0); // nuclei
				cleanChannel("ch"+ channel +"_2d", min, max); 
				...
				run("Merge Channels...", "c1=[ch1_2d] c"+ channel + "=[ch"+ channel + "_2d] create");
				 ...
				 // 8. Save and Close
				 saveAs("Tiff", outputDir + File.nameWithoutExtension + "_series" + (s) + "_ch"+ channel +"_with_nuclei_overlay.tif");
	}
...
//function createOverlayOfChannelWithNuclei(channel, min, max){
//	// 2. Set Batch Mode for speed
//	// 20x faster; bypassess overhead of rendering images to the display.
//	setBatchMode(true); 
//
...
//			     // 7. Merge Channels  
//			    // Merge Channels works on OPEN IMAGE WINDOWS, not filenames. Therefore, selectImage() the needed one
//			    run("Merge Channels...", "c1=[ch1_2d] c" + channel +"=[ch"+ channel +"_2d] c" create");
//			    
//			    // 8. Save and Close
//			    saveAs("Tiff", outputDir + File.nameWithoutExtension + "_series" + (s) + "_ch"+ channel1 +"_ch"+ channel2 +"_with_nuclei_overlay.tif");
//
//	 			close("*"); // Closes all images
//	}
//	
